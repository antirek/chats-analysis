...
## Ключевые понятия OpenIM

### 1. Conversation (Беседа)

**Conversation** - это центральное понятие в OpenIM, представляющее диалог между пользователями или в группе. Каждая беседа имеет уникальный идентификатор и содержит метаданные о диалоге.

**Структура Conversation:**
- `ConversationID` - уникальный идентификатор беседы
- `ConversationType` - тип беседы (личный чат, групповой чат, уведомления)
- `OwnerUserID` - владелец беседы (для личных чатов)
- `UserID` / `GroupID` - участники беседы
- `MaxSeq` / `MinSeq` - диапазон последовательностей сообщений
- `RecvMsgOpt` - настройки получения сообщений
- `IsPinned` - закреплена ли беседа
- `IsPrivateChat` - приватный чат
- `GroupAtType` - тип упоминания в группе (@me, @all, @all@me)

**Формат ConversationID:**
- Личный чат: `si_{userID1}_{userID2}` (отсортированные ID)
- Групповой чат: `sg_{groupID}`
- Уведомления: `n_{userID1}_{userID2}` или `n_{groupID}`
- Серверные уведомления: `sn_{userID1}_{userID2}`

### 2. SessionType (Тип сессии)

Определяет тип коммуникации в сообщении:

- **`SingleChatType`** (1) - Личный чат между двумя пользователями
- **`ReadGroupChatType`** (2) - Групповой чат (только чтение для некоторых участников)
- **`WriteGroupChatType`** (3) - Групповой чат с возможностью записи
- **`NotificationChatType`** (4) - Уведомления (системные сообщения)

### 3. Seq (Sequence Number)

**Seq** - это порядковый номер сообщения в беседе. Каждое сообщение получает уникальный последовательный номер в рамках беседы.

**Типы Seq:**
- **MaxSeq** - максимальный номер сообщения в беседе
- **MinSeq** - минимальный номер сообщения в беседе (для оптимизации хранения)
- **HasReadSeq** - последний прочитанный номер сообщения для конкретного пользователя
- **ConversationUserMinSeq** - минимальный seq для пользователя в беседе

**Назначение:**
- Синхронизация сообщений между клиентами
- Определение пропущенных сообщений
- Оптимизация загрузки истории
- Управление хранением (удаление старых сообщений)

### 4. Идентификаторы сообщений

**ClientMsgID** - идентификатор сообщения, сгенерированный клиентом. Используется для:
- Дедупликации сообщений
- Отслеживания статуса отправки
- Синхронизации между устройствами

**ServerMsgID** - идентификатор сообщения, присвоенный сервером. Генерируется на основе:
- ClientMsgID
- SendID (ID отправителя)
- Timestamp
- Случайное число

Формат: `MD5(timestamp + "-" + sendID + "-" + random)`

### 5. PlatformID (Идентификатор платформы)

Определяет платформу, с которой отправлено сообщение:

- **`iOSPlatformID`** (1) - iOS приложение
- **`AndroidPlatformID`** (2) - Android приложение
- **`WindowsPlatformID`** (3) - Windows приложение
- **`OSXPlatformID`** (4) - macOS приложение
- **`WebPlatformID`** (5) - Web приложение
- **`MiniWebPlatformID`** (6) - Мини-программа (WeChat Mini Program)
- **`LinuxPlatformID`** (7) - Linux приложение
- **`AndroidPadPlatformID`** (8) - Android планшет
- **`IPadPlatformID`** (9) - iPad
- **`AdminPlatformID`** (10) - Административная панель

**Использование:**
- Мультитерминальная синхронизация
- Управление онлайн статусом
- Настройки push уведомлений для разных платформ

### 6. Message Options (Опции сообщения)

Опции сообщения определяют поведение обработки и доставки сообщения:

**Основные опции:**

- **`IsHistory`** - сохранять ли сообщение в историю
  - `true`: сообщение сохраняется в MongoDB
  - `false`: сообщение только для push, не сохраняется

- **`IsPersistent`** - постоянное хранение
  - Используется для важных уведомлений
  - Гарантирует сохранение даже при сбоях

- **`IsOfflinePush`** - отправлять ли офлайн push уведомление
  - `true`: отправлять push, если пользователь офлайн
  - `false`: не отправлять push уведомления

- **`IsSenderSync`** - синхронизировать ли с отправителем
  - `true`: отправитель также получает сообщение (для синхронизации между устройствами)
  - `false`: только получатель получает сообщение

- **`IsUnreadCount`** - увеличивать ли счетчик непрочитанных
  - `true`: увеличивать счетчик непрочитанных сообщений
  - `false`: не изменять счетчик

- **`IsConversationUpdate`** - обновлять ли беседу
  - `true`: обновлять последнее сообщение в беседе
  - `false`: не обновлять метаданные беседы

- **`IsNotNotification`** - не является ли уведомлением
  - `true`: обычное сообщение
  - `false`: системное уведомление

- **`IsSendMsg`** - отправлять ли как сообщение
  - Используется для уведомлений, которые также должны отображаться как сообщения

### 7. ContentType (Тип контента)

Определяет тип содержимого сообщения:

**Основные типы:**
- **`Text`** (101) - Текстовое сообщение
- **`Picture`** (102) - Изображение
- **`Voice`** (103) - Голосовое сообщение
- **`Video`** (104) - Видео
- **`File`** (105) - Файл
- **`AtText`** (106) - Текст с упоминаниями (@user)
- **`Custom`** (107) - Пользовательский тип
- **`Quote`** (108) - Цитата
- **`MarkdownText`** (109) - Markdown текст
- **`Card`** (110) - Карточка
- **`Location`** (111) - Геолокация
- **`BusinessCard`** (112) - Визитная карточка
- **`Merge`** (113) - Объединенные сообщения
- **`Emoji`** (114) - Эмодзи

**Типы уведомлений:**
- **`HasReadReceipt`** (1001) - Подтверждение прочтения
- **`GroupCreatedNotification`** (1501) - Группа создана
- **`GroupInfoSetNotification`** (1502) - Информация о группе изменена
- **`MemberQuitNotification`** (1504) - Участник покинул группу
- **`MemberInvitedNotification`** (1505) - Участник приглашен
- И многие другие...

### 8. Online/Offline Status (Онлайн/Офлайн статус)

**Онлайн статус** - информация о том, подключен ли пользователь к серверу через WebSocket.

**Механизм:**
- При подключении WebSocket пользователь помечается как онлайн в Redis
- При отключении статус обновляется
- Используется для определения способа доставки сообщений:
  - **Онлайн**: доставка через WebSocket в реальном времени
  - **Офлайн**: доставка через push уведомления (FCM, JPush и др.)

**Хранение:**
- Redis кэш с TTL
- Ключ: `ONLINE_STATUS:{userID}:{platformID}`
- Значение: информация о соединении

### 9. HasReadSeq (Прочитанная последовательность)

**HasReadSeq** - номер последнего прочитанного сообщения пользователем в беседе.

**Использование:**
- Определение непрочитанных сообщений
- Синхронизация статуса прочтения между устройствами
- Отображение индикаторов прочтения
- Оптимизация загрузки (загружать только непрочитанные)

**Хранение:**
- Redis: `HAS_READ_SEQ:{userID}:{conversationID}`
- MongoDB: для долгосрочного хранения

### 10. Message Status (Статус сообщения)

Статус отправки сообщения:

- **`MsgStatusSending`** (1) - Отправляется
- **`MsgStatusSendSuccess`** (2) - Успешно отправлено
- **`MsgStatusSendFailed`** (3) - Ошибка отправки

### 11. RecvMsgOpt (Настройки получения сообщений)

Настройки пользователя для получения сообщений:

- **`ReceiveMessage`** (0) - Получать все сообщения
- **`NotReceiveMessage`** (1) - Не получать сообщения (игнорировать)
- **`ReceiveNotNotifyMessage`** (2) - Получать сообщения, но без уведомлений

**Уровни настройки:**
- **Глобальные настройки** - для всех бесед пользователя
- **Настройки беседы** - для конкретной беседы (приоритет выше глобальных)

### 12. GroupAtType (Тип упоминания в группе)

Определяет тип упоминания пользователя в групповом чате:

- **`AtNormal`** (0) - Обычное сообщение (без упоминаний)
- **`AtMe`** (1) - Упоминание конкретного пользователя (@user)
- **`AtAll`** (2) - Упоминание всех (@everyone)
- **`AtAllAtMe`** (3) - Упоминание всех и конкретного пользователя

### 13. ConversationUniqueKey

**ConversationUniqueKey** - уникальный ключ для идентификации беседы в очередях и кэше.

**Формат:**
- Личный чат: `{userID1}_{userID2}` (отсортированные ID)
- Групповой чат: `{groupID}`

**Использование:**
- Шардирование сообщений в очереди
- Группировка сообщений для батч обработки
- Ключ для Redis кэша

### 14. Message Batching (Батчинг сообщений)

**Батчинг** - группировка сообщений для эффективной обработки.

**Параметры батчинга:**
- **Size** (500) - максимальное количество сообщений в батче
- **Interval** (100ms) - максимальное время ожидания перед обработкой батча
- **Worker** (50) - количество воркеров для обработки

**Преимущества:**
- Снижение нагрузки на БД
- Улучшение производительности
- Гарантия порядка обработки (в рамках беседы)

### 15. Webhook Events (События Webhook)

OpenIM поддерживает webhook'и на различных этапах обработки сообщений:
...
### Схема взаимосвязи ключевых понятий

```mermaid
graph TB
    USER[User<br/>Пользователь]
    CONV[Conversation<br/>Беседа]
    MSG[Message<br/>Сообщение]
    SEQ[Seq<br/>Последовательность]
    OPT[Options<br/>Опции]
    
    USER -->|создает| CONV
    CONV -->|содержит| MSG
    MSG -->|имеет| SEQ
    MSG -->|имеет| OPT
    MSG -->|имеет| CT[ContentType<br/>Тип контента]
    MSG -->|имеет| CMID[ClientMsgID<br/>ID клиента]
    MSG -->|имеет| SMID[ServerMsgID<br/>ID сервера]
    
    USER -->|имеет| PID[PlatformID<br/>Платформа]
    USER -->|имеет| STATUS[Online/Offline<br/>Статус]
    USER -->|имеет| HASREAD[HasReadSeq<br/>Прочитано]
    
    CONV -->|имеет| CONVID[ConversationID<br/>ID беседы]
    CONV -->|имеет| STYPE[SessionType<br/>Тип сессии]
    CONV -->|имеет| MAXSEQ[MaxSeq<br/>Макс. seq]
    CONV -->|имеет| MINSEQ[MinSeq<br/>Мин. seq]
    
    MSG -->|отправляется через| WS[WebSocket<br/>или HTTP]
    MSG -->|сохраняется в| REDIS[(Redis<br/>Кэш)]
    MSG -->|сохраняется в| MONGO[(MongoDB<br/>БД)]
    MSG -->|доставляется через| PUSH[Push Service<br/>Сервис доставки]
    
    style CONV fill:#fff4e1
    style MSG fill:#e1f5ff
    style SEQ fill:#ffe1f5
